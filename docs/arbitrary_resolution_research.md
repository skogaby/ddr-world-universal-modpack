...
| 7 | `FUN_18021afb0` | get render context |
...
rasterise at native resolution; bitmaps are upsampled from their 720p atlases.
...
### Tier C — hi-res asset pass (open-ended)

Per-asset replacement of 720p bitmaps with 2× (or 3× for 4K) art through the
existing LayeredFS texture pipeline: arrow sheets (768×384 → 1536×768; UVs are
normalised so same-layout sheets drop in, but the AA PS `TEXEL` constants must
be parametrised), judgement/combo art, banners/jackets (source quality
permitting), and eventually fonts (KBF atlases — new hi-res KBFs via the
`kbf_font_format.md` tooling; the `scr_distancefont` SDF fonts already scale).
This is where "manually repositioning/resizing UI" would live, but note that
**no repositioning is needed** — layout is logical; only pixel density of the
art changes. **Effort: weeks-to-months of art work, incremental, optional.**
...
- `shader_fixes` AA PS: its "collapses to stock at 1:1" identity no longer holds
  (by design it then does real filtering); `TEXEL` constants only matter with
  hi-res sheets (Tier C).

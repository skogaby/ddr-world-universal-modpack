...
|---|---|---|
| Panel getters Up/Down/Left/Right | +0x310/318/320/328 → `FUN_1800c9a30/9900/97d0/96a0` | `u64 impl(this, player, *state_u8, *trigger_u8, *press_ts_u64, *release_ts_u64)` — FIVE out-args (deploy #1 boot crash came from assuming 3). The two u64s are the panel's press/release TIMESTAMPS in the libavs ord-45 clock (P1 `+0x630../+0x670..`, P2 `+0x650../+0x690..`), derived by the ark from libacio2's MDXF poll ring — NOT sensor levels (corrected 2026-09-11, `docs/input_polling_research.md` §4). Players 4..=11 = debug-keyboard rows. Injection: OR level into `state`, latch-synthesized rising edge into `trigger`, and backfill a ZERO timestamp with the current ord-45 time while held/releasing (nonzero = the ark's own stamp, left alone). |

...
Live per-player stats while you play: millisecond error (current/max/mean), EX loss, calories burned — plus an option to replace the pacemaker with your latest ms-error, and per-song CSV export of your step data. Signed readouts — on screen and in the CSV's `Delta (Ms Error)` column — use the game's own results convention: positive = FAST (early), negative = SLOW (late); misses don't count as timing samples. The stat blocks stay up through the results screen so you can read the breakdown after the song. They are centre-aligned and mirrored for P1/P2; their size (50–150 %) and position (horizontal shift inward/outward, vertical shift — anywhere on the screen) are adjustable from the mod menu's GLOBAL SETTINGS tab, so you decide how much of the play area they occupy.

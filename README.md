...
Playing alone? Turn on **BOT OPPONENT (1P ONLY)** in the options menu, pick a **BOT LEVEL** from `Level 1` to `Level 10`, and the next song is a genuine two-player VERSUS session — the empty pad is taken by a computer opponent that plays your exact chart, at your speed mod and lane options, with a `BOT LV<n>` name plate on its side of the screen.

The level is real skill, not a score multiplier. Every arrow the bot hits is judged by the game's actual judge on a press placed by a timing model of a dancer who rushes some phrases and drags others, has a sharp "in the pocket" step and a loose one, and an off day now and then — so its Marvelous / Perfect / Great spread, its FAST/SLOW readout and its S-Marvelous count all look like a person's, not a metronome's. A level-10 bot full-combos essentially every chart, gets a **Perfect** Full Combo almost every time and a **Marvelous** one about one song in ten; a level-1 bot scatters Perfects, Greats, Goods and Misses, fails roughly one song in ten, and hardly ever fails a Beginner chart. In between, each step up is a noticeably better dancer. Quick restart re-rolls the bot with a fresh pattern, so no two attempts play out the same.

One step past `Level 10` is **TARGET SCORE**: the bot becomes a replay of whatever your pacemaker is pointed at — your own best, a rival's score, the world record — recreated step for step from the ghost data the game already downloaded for that target. Its name plate shows the name of the player who set that score (your own name when you're chasing your best), with a small **TARGET BOT** tag above it during the song and on the results screen so a screenshot never passes for that player's real play; when the game can't tell whose score it is, the plate just reads `TARGET`. Because ghost data only records grades — it predates S-Marvelous — the replay side never shows S-Marvelous: its judgements, FAST/SLOW (Marvelous doesn't count) and results pane stay stock, with `-` in the S-MARVELOUS row. Charts with no target score yet fall back to a `Level 10` opponent with a short on-screen notice. The bot's results pane always opens on the **Details** tab (judgement counts and FAST/SLOW). Both bot settings are remembered on the cabinet only; they are never sent to or loaded from the server.
...
And the scene doesn't have to look like 2013. **LIGHTING STYLE** (mod menu, `GLOBAL SETTINGS` → `BACKGROUND DANCERS`) shades the whole stage and its dancers under a key light: **STOCK (UNLIT)** is the game's flat look, **SMOOTH SHADING** adds form to arms, faces and props, and **CEL SHADING** turns the same light into three toon bands with rim ink — plus **SCENE OUTLINES**, an ink outline around every prop and dancer, with separate outline widths for dancers and stage.

Songs with a background movie get their own choice, **BACKGROUND MOVIES** (same menu section, next song): **OFF** skips the movie so the stage plays as on any other song, **THUMBNAIL** plays it in its small window over the stage, **STAGE SCREENS** (the default) is the DDR A3 look — when the song's stage has video screens (the ten stock `monitor` / `replicant` stages, e.g. ENDYMION's `replicant05`, the shipped Griffin House's TV, and any custom stage whose screen image is named `offscreen1` — see the Blender add-on README), the movie plays on those screens with the whole stage and the dancers around it (other stages play the song as THUMBNAIL; a song without a movie leaves the screens black), **MOVIE ONLY (NO DANCERS)** is DDR A3's default for ordinary movie songs — while the movie plays the whole 3D scene steps aside and the song looks like stock World (songs without a movie still get the dancers), and **FULLSCREEN (NO STAGE)** is the DDR 5th Mix look — the movie fills the screen behind the dancers and the 3D stage steps aside for that song, filmed by a camera set of its own: close, front-facing shots that keep the dancers the focus of the picture (the clips live in `data_mods/background_dancers/movie_camera/` and are generated by `scripts/gen_movie_cameras.py` — `_1p` clips are used with one dancer, `_2p` clips with two, `_non` clips are the short cut-aways at each dance change). It applies to every player with movies turned on (a VIDEO SIZE of OFF stays off), and the player's saved VIDEO SIZE is never changed.
...
### DDR SELECTION
DDR A3's DDR SELECTION is back as an option: set **DDR SELECTION** in the in-game OPTIONS menu (or the mod menu's `PLAYER SETTINGS`) to an era — **1st-5th**, **MAX-EXTREME**, **SuperNOVA**, **X** or **2013-A** — and every song plays with that era's gameplay screen; **AUTO** gives each song the look of the DDR version it first appeared in (1st MIX through DDR A; A20 and later keep World's screen). The legacy art is loaded from files the stock World install already ships. One skin applies per song for the whole cabinet (P1's choice in versus); courses stay stock. The mod is on by default but changes nothing until a player picks an era (`"ddr-selection": false` in `mods` turns it off entirely). With an era picked, the judgement words, FAST/SLOW, full-combo splash, game-over, danger, life-gauge, combo, score and difficulty art change and sit where the era put them (the 2P gauge mirrored and the 1st-5th / 2013-A gauges filling in cells, the combo growing with the count and — on X and 2013-A — taking the colour of the combo's worst judgement, as in A3), the era's stage frame ("1st STAGE" …) replaces World's, World's BPM display and player name are hidden (MAX-EXTREME gets its song-info band, SuperNOVA / X / 2013-A A3's song title panel), the pacemaker uses A3's digits on every era and the option icons are A3's, the song intro plays the era's READY / HERE WE GO, the screen before each song is A3's stage panel with the era's cut-in and stage call, the song ends on the era's CLEARED / FAILED banner (Tohoku EVOLVED gets PRAY FOR ALL on the X era), and the eighteen songs that had a DDR SELECTION movie in A3 (TRIP MACHINE, PARANOiA, AFRONOVA, MAX 300, CANDY☆, …) play it as their background movie when VIDEO SIZE shows one; the announcer and crowd follow A3's rules for the era (the DDRMAX-era calls on MAX-EXTREME / SuperNOVA, A3's own announcer on X / 2013-A). On **1st-5th**, as in A3, every player plays the song with the classic options — speed ×1.00, no BOOST, arrows visible, step zone on, normal scroll, FLAT colour, CLASSIC arrows, no lane filter, no guideline (the in-song speed change still works, starting from ×1.00); your own options come back right after the song and are never saved changed. With **S-Marvelous Judgement** on, every era gets its S-Marvelous in the era's own style: a violet version of the era's MARVELOUS word (following the Judgement Color setting; the era's own MARVELOUS keeps its A3 shimmer), a violet S-MFC full-combo splash, and on X and 2013-A — the eras whose combo takes the colour of the worst judgement — a violet combo while it is all S-Marvelous.

The era cut-in (the big animation naming the song's DDR era, before the stage panel) adds a few seconds to every song start. Press START to skip it, or turn it off for everyone under the mod menu's `GLOBAL SETTINGS` → **DDR SELECTION** → **Era Cut-In** (saved as `"ddr_selection": {"era_cutin": false}`; applies from the next song).

One file the 2013-A skin's combo counter needs is blank in World's data (`dance_combo0005_v0.arc`; without it the 2013-A skin keeps World's combo). If you also have a DDR A3 install, copy it over once — from the game folder:

```
ddr_selection_import\import_a3_assets.bat "D:\path\to\ddr_a3\contents"
```

(`import_a3_assets.sh` on macOS/Linux; the path is the A3 folder that contains `data\`). It copies into `data_mods\ddr_selection_a3\` and never touches World's own files.
...
This mod replaces the music clock with one driven by the sound card's **actual playback position**, latched to the exact sample the song started on: every play starts at the same offset and never drifts, including after quick restarts and Training Mode scrubs. If you'd prefer the sound card to only fix the startup offset and leave the in-song clock to the game (see `audio_clock.mode: "anchor"` in the configuration table), that's a one-line config change.
...
A brand-new judgement tier above Marvelous: steps landed within **±12 ms** earn a violet **S-Marvelous**, with the full native treatment — its own judgement flash, combo colors, full-combo splash, a dedicated row on the results screen, its own series on the play graph, and S-MFC emblems for an all-S-Marvelous full combo (on DDR SELECTION's legacy eras too, in each era's style). It's pure presentation: to the game (and the network) an S-Marvelous is still a Marvelous, so your scores and records are completely untouched.
...
Live per-player stats while you play — plus an option to replace the pacemaker with your latest ms-error, and per-song CSV export of your step data. Two content sets: **DETAILED** (millisecond error current/max/mean, EX loss, calories burned) or **STREAMLINED** (Δ of the last step, the largest Δ so far with its sign, EX loss, then your running S-Marvelous* / Marvelous / Perfect / Great / Good / Miss counts). Three layouts: a **SIDE COLUMN** beside each playfield (one field per line), a single **BOTTOM LINE** along the bottom edge of the screen, in the band the stock CREDIT / PASELI text normally uses (that text is hidden while the line is on screen; P1's line reads from the left edge, P2's ends at the right), or the same single line as a **TOP LINE** along the top edge (the stock text stays). Signed readouts — on screen and in the CSV's `Delta (Ms Error)` column — use the game's own results convention: positive = FAST (early), negative = SLOW (late); misses don't count as timing samples. The stat blocks stay up through the results screen so you can read the breakdown after the song. They are mirrored for P1/P2; their size (50–150 %) and position (horizontal shift inward/outward, vertical shift — anywhere on the screen) are adjustable from the mod menu's GLOBAL SETTINGS tab, the side column's line alignment too, and each layout remembers its own position, so you decide how much of the play area they occupy.

\* The S-Marvelous column appears only when the S-Marvelous Judgement mod is enabled; Marvelous then counts the Marvelous steps that were NOT S-Marvelous, so the two add up to your total.
...
| **DDR SELECTION** | A3's legacy gameplay skins per player (OFF / AUTO / 1st-5th / MAX-EXTREME / SuperNOVA / X / 2013-A), from the stock install's own legacy data (plus a one-time A3 import for 2013-A's combo). On by default; nothing changes until a player picks an era. |

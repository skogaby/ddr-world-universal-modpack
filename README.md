...
Playing alone? Turn on **BOT OPPONENT (1P ONLY)** in the options menu, pick a **BOT LEVEL** from `Level 1` to `Level 10`, and the next song is a genuine two-player VERSUS session — the empty pad is taken by a computer opponent that plays your exact chart, at your speed mod and lane options, with a `BOT LV<n>` name plate on its side of the screen. Both READY panels, both lanes, both gauges, both results panes: it is the game's own 2P mode, driven natively, with a player whose feet the modpack controls.
...
Prefer a particular dancer or stage? The game's own **OPTIONS** menu (PLAYFIELD STYLING OPTIONS) gains two rows: **BACKGROUND DANCER** picks your dancer (`RANDOM` or any of the 26 — `EMI #2`, `RAGE #1`, …) and **BACKGROUND STAGE** picks the stage (`RANDOM` or any of the 25 — `BOOM #3`, `CRYSTALDIUM`, …). While a row is focused with a specific value, the options preview box shows that dancer dancing (or that stage under its own camera cuts) **live in 3D** — rendered by the game's model passes right inside the menu — so you can browse the whole cast before the song. Each player picks their own dancer; the stage is shared by both sides in a 2P session. The choices are remembered on this machine only (never sent to a server) and take effect from the next song.

**Custom dancers and stages.** The cast is not limited to Konami's. Any character or stage built with the repo's Blender add-on (`tools/blender_ddr_addon/`) — or ripped from another game and ported with its playbook — can join the rotation: drop the add-on's exported folder into `data_mods/custom_models/`, inside a folder named after the character or stage, and it is treated exactly like the stock content — random picks, its own entry in the BACKGROUND DANCER / BACKGROUND STAGE rows (labelled `PETER GRIFFIN`, `GRIFFIN HOUSE`, …) and a live 3D preview. No archive packing: the modpack packs what the game needs itself (into `data_mods/_cache/`) the first time it sees a folder. The modpack ships with the first two, Peter Griffin and the Griffin living room:

```
data_mods/custom_models/dancers/Peter Griffin/pl_peter00/            the body: pl_peter00.model, pl_peter00.b2it, *.dds
data_mods/custom_models/dancers/Peter Griffin/pl_peter00_head00/     (optional accessory parts, same shape)
data_mods/custom_models/stages/Griffin House/mapset_griffin00/       the stage: one gm_griffin00_<part>/ folder per part …
data_mods/custom_models/stages/Griffin House/mapset_griffin00/camera/  … and its camera shots (griffin_st01.camanm, …)
```

A ready-made `pl_<key>.arc` / `mapset_<key>.arc` is accepted in the same place, and a folder may hold either the add-on's flat export or a literally unpacked arc (`data/chara/…`). The display name is the enclosing folder's name (a model placed directly in `dancers/` or `stages/` is named from its key, `_` → space); the key in the folder name must match the models inside, as the add-on exports them. A dancer with no extra information is treated as a male-choreography character at scale 1.0 / shadow 0.8; to change that, or to set a stage's part priorities, drop a `chara_resources.rlist` / `map_resources.rlist` / `stage_camera_resources.rlist` next to the model folder with a row for that key (the very rows a DDR A3 `startup.arc` carries — copy them over unchanged — or the plain-text twin `<name>.rlist.txt`, one `key, field, field, …` line per row).

A stage's camera is its own: every `.camanm` clip inside the stage folder becomes one shot of its camera set (clips whose name contains `_non` are the short cut-aways played at dance changes, the rest cycle as the main shots — the same stage-mode sequencing as the stock stages, whose shots run 6–7.5 s each). A stage that ships no clips borrows the first stock stage's shots, which is why a small room needs its own: `scripts/split_camanm.py` turns a single long camera clip authored the DDR A3 way (`tools/blender_ddr_addon/examples/stage_camera.py`, hard cuts baked in) into per-shot clips re-timed to the stock cadence — the Griffin House shots were made exactly so. (A stage needs no dance-pad model either — World's normal play never shows one; the `footpanel` part in the add-on's A3 recipe was for the A3 tutorial demo only.) The **Custom Dancers & Stages** row under BACKGROUND DANCERS in the mod menu turns the whole feature off; like the rest of the catalog it is read at launch, so that switch applies at the next start.

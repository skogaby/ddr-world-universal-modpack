...
/// Word-clone options: mute the stock `marvelous_ef` additive self-glow.
/// Stock `dance_marvelous` stamps the word twice — normal + additive
/// (blend 8) with mult alpha pulsing 0.20 → 0.098 → 0 every ~4–5 frames.
/// Additive blending clamps on white letters (invisible in stock); on the
/// violet S-Marv art it is a ~12–15 Hz flicker (maintainer video
/// 2026-09-03, art-only mitigation insufficient). The S-Marv copy is ALWAYS
/// muted; `mute_stock_glow` additionally silences the stock Marvelous word's
/// own pulse — the SHIPPED shape since 2026-09-13 (the former "Marvelous
/// Shimmer" ON/OFF row is retired: the pulse was judged ugly outright, so
/// the stock word now renders static like the S-Marv word). `false` is the
/// fallback rung of [`run_word_clone`]'s ladder only.

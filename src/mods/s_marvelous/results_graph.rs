...
//! (design §4.8, plan Step 8) and, on the TIMING page, the Marvelous
//! FAST/SLOW series + legend entries (2026-09-10 follow-up).
//!
...
//! and legend text EVERY FRAME. Four cooperating detours, zero absolute
...
//!    content, clamped), build the LOOSE-Marvelous FAST/SLOW vectors for
//!    the timing page ([`records::marvelous_fast_slow_per_second`] —
//!    stock never draws grade 0 there because Marvelous was the exempt
//!    top tier; with the mod on that tier is S-Marvelous), and register
//!    the tab. Also resets the per-frame legend state machine.
...
//!    append our violet series — the callable's lambda vftable is captured
//!    live from the incoming argument BEFORE the original consumes it
//!    (same `(uint,double,double)` family, so a `{vft, violet}` clone of
//!    it is a valid color functor). Our series therefore LEADS the
//!    judge-colored series (S-Marv first — maintainer directive
//!    2026-08-30). On the TIMING page, after the FAST PERFECT append
//!    (`vec == tab+0x3F8`) / SLOW PERFECT append (`vec == tab+0x438`) our
//!    Marvelous FAST/SLOW series are appended LAST — the bar renderer
//!    stacks the last-appended series at the axis, so Marvelous sits
//!    between the two PERFECT bands exactly where the best tier belongs.
//!    The timing lambdas are HSV-shifting functors (`cdfd0(rgba@+0xC, 0,
//!    0, value_shift)` with the shift baked into the lambda's code), so we
//!    capture the GREAT (shift 0 = identity) lambda's vftable from the
//!    `tab+0x3D8` / `tab+0x458` appends of the same frame and carry our
//!    rgba at +0xC. Re-injection every frame is automatic (charts are
//!    rebuilt per frame).
//! 3. **Legend detour** (`graph_legend_text`): judge page — when the
...
//! 4. **Axis detour** (`graph_timing_axis_max`, post-original): the
...
/// family: the JUDGE lambdas return the rgba stored at +0x8 (`word_lo`);
/// the TIMING lambdas run `cdfd0(rgba@+0xC, 0, 0, shift)` — rgba at
/// `word_hi`, `word_lo` unused (stale stack in the stock caller).
#[repr(C)]
...
/// all-or-nothing; the timing axis detour is optional and gates the
/// timing-page half. Fail-open.
...
#[derive(Clone, Copy)]
enum AppendRole {
    /// Judge page filler (`tab+0x538`): inject the violet series next.
    JudgeFiller,
...
unsafe extern "C" fn append_hook(
    ...
    // POST-original: judge FILLER ⇒ our violet series is appended right
    // after it, BEFORE the shimmer/marvelous pair — the S-Marv tier leads
    // the judge stack (maintainer directive 2026-08-30: S-Marv first).
    // Timing PERFECT ⇒ our Marvelous band is appended LAST = at the axis.
    if vft != 0 {
...
fn classify_append(vec: *mut MsvcVec<f64>) -> Option<(*mut u8, AppendRole)> {
    let guard = TABS.lock().ok()?;
    let map = guard.as_ref()?;
    for (offset, role) in APPEND_ROLES {
...
/// judge FILLER ⇒ append our S-Marv series (the functor vftable was
/// live-captured from the incoming argument — same `(uint,double,double)`
/// lambda family, so a `{vft, violet}` clone is a valid color functor);
/// timing GREAT ⇒ remember its identity-shift vftable; timing PERFECT ⇒
/// append the Marvelous FAST/SLOW band with that vftable. Our injected
/// call re-enters the hook but fails the vec-identity gate (its source is
/// our stack view).
fn maybe_inject_series(chart: *mut u8, vec: *mut MsvcVec<f64>, vft: usize) {
    unsafe {
        ...
        let Some((tab, role)) = classify_append(vec) else {
            return;
        };

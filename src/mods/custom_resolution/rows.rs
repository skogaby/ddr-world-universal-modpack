...
//! operator's `presets`) and SD PRESENT MODE (CROP / LETTERBOX — 4:3 outputs
//! only). Both persist the WHOLE `resolution` section through
...
//! enum-row shape.
//!
//! The render size and the AA config are NOT exposed: the plan derives both
//! from the output (`plan.rs` module docs) so a cabinet can never be
//! configured into the slower present chain.
...
/// Load the section, normalize the choice list (valid `WxH` presets, the
...
pub fn register() {
    load();
    let (outputs, cur_out, cur_sd) = match STATE.lock() {
        Ok(st) => (
            st.outputs.clone(),
    ...
    mod_menu::register_enum_row(mod_menu::EnumRowSpec {
        ...
        hint: "Back-buffer / window size; 16:9 sizes render natively, 4:3 sizes use the SD-cabinet present path. Restart the game to apply."
            .to_string(),
...
pub fn unregister() {
    mod_menu::remove_rows_for(&[ROW_OUTPUT, ROW_SD_PRESENT]);
}
